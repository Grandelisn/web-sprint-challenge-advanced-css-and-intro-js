...
function getArtistByIndex(testArray, index) {
  return `the artist at index ${testArray[index].id} is ${testArray[index].name}`
}  
...
function get20s(testArray){
  const newArray = [];
  for(let i = 0; i<testArray.length; i++){
    const index = testArray[i];
    const indexBirth = parseInt(index.years.substring(0,4));
    const indexDeath = parseInt(index.years.substring(7));
    if(indexBirth > 1900 && indexDeath < 2000){
      newArray.push(testArray[i].name);
    }
...
function removeArtist(testArray, index){
...
function addArtist(testArray){
    const newObj = { 
      ...
      bio: 'When I was a boy I had a friend steve blade'
    } 
...
function lotsOfArt(testArray){
  const newArray = [];
  for(let i = 0; i < testArray.length; i++){
    if(testArray[i].paintings>100){
      newArray.push(testArray[i].name);
    }
  }
  return newArray;
